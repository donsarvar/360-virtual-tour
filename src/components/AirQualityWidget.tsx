...
export const AirQualityWidget = ({ lat = 41.3111, lon = 69.2406, className = "" }: { lat?: number, lon?: number, className?: string }) => {
  ...
  const translations = {
    ...
    live: { uz: "Jonli", ru: "Jonli", en: "Live" },
    pm25: { uz: "PM2.5 ko'rsatkichi", ru: "Показатель PM2.5", en: "PM2.5 Level" },
    source: { uz: "Manba", ru: "Источник", en: "Source" },
    description: {
      uz: "Toshkent markazi uchun real vaqtdagi havo sifati indeksi.",
      ru: "Индекс качества воздуха в центре Ташкента в реальном времени.",
      en: "Real-time air quality index for Tashkent city center."
    },
    mobileDesc: {
      uz: "Toshkent shahri havo sifati indeksi. Datchik ma'lumotlariga asoslangan.",
      ru: "Индекс качества воздуха Ташкента. На основе данных датчиков.",
      en: "Tashkent City Air Quality Index. Based on sensor data."
    }
  };

  const l = (key: keyof typeof translations) => translations[key][lang as "uz" | "ru" | "en"];
        ...
        {expanded && (
          ...
          >
            <div className="flex justify-between items-center mb-2 pb-2 border-b border-white/10">
              <span className="text-xs text-white/50 uppercase tracking-widest font-bold">{l('details')}</span>
              <span className={`text-xs font-bold ${getStatusTextColor(data.status)}`}>{l('live')}</span>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs text-white/70">{l('pm25')}:</span>
                <span className="text-xs font-bold text-white">{data.pm25} μg/m³</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-white/70">{l('source')}:</span>
